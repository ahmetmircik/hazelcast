...
public class MapClearExpiredRecordsTask extends AbstractClearExpiredRecordsTask<PartitionContainer>
    ...
    private void doBackupExpiration(PartitionContainer container) {
        ...
        for (RecordStore recordStore : maps.values()) {
            ((AbstractEvictableRecordStore) recordStore).sendExpiredKeysToBackups(false, false);
        }
    }
    ...
    protected void sendBackupEqualizer(PartitionContainer container) {
        if (nodeEngine.getPartitionService().getPartition(container.getPartitionId()).isLocal()) {
            ConcurrentMap<String, RecordStore> maps = container.getMaps();
            for (RecordStore recordStore : maps.values()) {
