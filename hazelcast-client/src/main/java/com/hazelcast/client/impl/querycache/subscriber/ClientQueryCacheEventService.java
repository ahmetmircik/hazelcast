...
public class ClientQueryCacheEventService implements QueryCacheEventService {
    ...
    public boolean hasListener(String mapName, String cacheUuid) {
        ...
        Collection<ListenerInfo> infos = queryCacheToListenerMapper.getListenerInfos(cacheUuid);
        if (infos.isEmpty()) {
            return false;
        }
        return true;
    ...
    @Override
    public void publish(String mapName, String cacheUuid, Object event, int orderKey) {
        checkHasText(mapName, "mapName");
        checkHasText(cacheUuid, "cacheUuid");
        checkNotNull(event, "event cannot be null");

        Collection<ListenerInfo> listeners = getListeners(mapName, cacheUuid);
        for (ListenerInfo info : listeners) {
            try {
                ...
                logger.warning("EventQueue overloaded! Can not process IMap=[" + mapName + "]"
                        + ", QueryCache=[ " + cacheUuid + "]" + ", Event=[" + event + "]");
            }
        }
    }

    @Override
    public String listenPublisher(String mapName, String cacheUuid, ListenerAdapter adapter) {
        final String listenerName = generateListenerName(mapName, cacheUuid);
    ...
    public String addListener(String mapName, String cacheUuid, MapListener listener, EventFilter filter) {
        ...
        return queryCacheToListenerMapper.addListener(cacheUuid, listenerAdaptor, filter);
    }
    ...
    public boolean removeListener(String mapName, String cacheUuid, String id) {
        ...
        QueryCacheToListenerMapper queryCacheToListenerMapper = getOrPutIfAbsent(registrations, mapName, REGISTRY_CONSTRUCTOR);
        return queryCacheToListenerMapper.removeListener(cacheUuid, id);
    }
