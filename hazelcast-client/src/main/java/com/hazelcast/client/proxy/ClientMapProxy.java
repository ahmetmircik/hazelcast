...
public class ClientMapProxy<K, V> extends ClientProxy
    ...
    protected InternalCompletableFuture invokeContainsKeyOperation(Object key) {
        Data keyData = toData(key);
        ClientMessage message = MapContainsKeyCodec.encodeRequest(name, keyData, getThreadId());
        ClientInvocationFuture future = invokeAsync(message, keyData);
        return new ClientDelegatingFuture<V>(future, getSerializationService(), CONTAINS_KEY_RESPONSE_DECODER, false);
    }
    ...
    protected InternalCompletableFuture invokeGetAllOperation(Collection<Data> dataKeys) {
        ...
        final SimpleCompletableFuture<Map<Integer, Object>> future = new SimpleCompletableFuture<Map<Integer, Object>>(executor, logger);
        ...
        List<Future<ClientMessage>> futures = new ArrayList<Future<ClientMessage>>(expectedOperationCount);
